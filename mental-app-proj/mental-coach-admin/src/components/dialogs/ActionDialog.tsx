...
import { BaseDialogProps, Action } from "../../utils/types";
...
type Props = BaseDialogProps & { selectedItem: Action | null };

function ActionDialog({ open, onClose, selectedItem }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const res = await appFetch(
        isCreationMode ? "/api/actions" : `/api/actions/${selectedItem!._id}`,
        {
      ...
      enqueueSnackbar(isEditMode ? "השינויים נשמרו" : `פעולה חדשה נוצרה`);
      onCloseHandler();
      mutate("/api/actions");
      reset();
    } catch (error: any) {
        ...
        <Toolbar>
          ...
          <Typography sx={{ ml: 2, flex: 1 }} variant="h6" component="div">
            {isEditMode
              ? "ערוך מקרה"
              : isViewMode
              ? `פרטים של ${selectedItem.actionName}`
              : "צור מקרה"}
          </Typography>
          <IconButton
            edge="start"
